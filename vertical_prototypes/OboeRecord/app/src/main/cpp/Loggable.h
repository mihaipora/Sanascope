...
private:
    ...
    template <typename T>
    std::string concat(T fstVal){
        std::stringstream ss;
        ss << fstVal;
        return ss.str();
    }
    template <typename T, typename... Arguments>
    std::string concat(T fstVal,Arguments... args){
        std::stringstream ss;
...
public:
    ...
    template<typename... Arguments> void infoLog(Arguments... args){
        const char * msg = concat(args...).c_str();
        __android_log_print(ANDROID_LOG_INFO, logTag.c_str(), "%s", msg);
    };

    template<typename... Arguments> void errorLog(Arguments... args){
        const char * msg = concat(args...).c_str();
